...
export const handlers: R60Behavior['handlers'] = {
  ...
  chamberSetClimate:    (ctx: WorldContext<R60Instrument>, args: { temperatureDegC: number; humidityPercentRh?: number }) => { ctx.instrument.chamberSet(args.temperatureDegC, args.humidityPercentRh) },
  chamberSwitchOff:     (ctx: WorldContext<R60Instrument>) => { ctx.instrument.chamberOff() },
  chamberConfigureDevice: (ctx: WorldContext<R60Instrument>, args: Partial<ChamberSpec>) => { ctx.instrument.configureChamber(args) },
  indicatorConfigureDevice: (ctx: WorldContext<R60Instrument>, args: Partial<IndicatorSpec>) => { ctx.instrument.configureIndicator(args) },
